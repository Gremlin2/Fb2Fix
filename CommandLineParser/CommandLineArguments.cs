...
namespace FB2Fix.CommandLine
{
    ...
    public sealed class CommandLineParser
    {
        ...
        {
            return ParseArguments(arguments, destination, new ErrorReporter(Console.Error.WriteLine));
        }
        ...
        public static string ArgumentsUsage(Type argumentType)
        {
            ...
            if (NativeMethods.IsRunningOnWindows)
            {
                screenWidth = NativeMethods.GetConsoleWindowWidth();
            }
            else
            {
        ...
        private bool ParseConfigFile(string filename, object destination)
        {
            bool hadError;
            string[] nestedArguments;

            hadError = LexFileArguments(filename, out nestedArguments);
                    ...
                    if (argument.Length > 0)
                    {
                        switch (argument[0])
                        {
                            ...
                            case '-':
                                ...
                                int argumentPrefixLength = (NativeMethods.IsRunningOnWindows) ? 1 : 2;
                                int endIndex = argument.IndexOfAny(new char[] {':', '+', '-'}, argumentPrefixLength);
                                string option = argument.Substring(argumentPrefixLength, endIndex == -1 ? argument.Length - argumentPrefixLength : endIndex - argumentPrefixLength);
                                string optionArgument;

                                if (option.Length + argumentPrefixLength == argument.Length)
                                {
                                    optionArgument = null;
                                }
                                else if (argument.Length > argumentPrefixLength + option.Length && argument[argumentPrefixLength + option.Length] == ':')
                                {
                                    optionArgument = argument.Substring(option.Length + argumentPrefixLength + 1);
                                }
                                else
                                {
                                    optionArgument = argument.Substring(option.Length + argumentPrefixLength);
                                }
        ...
        public string GetUsageString(int screenWidth)
        {
            ...
            const int minimumNumberOfCharsForHelpText = 50;
            const int minimumHelpTextColumn = 30;
        ...
        private bool LexFileArguments(string fileName, out string[] arguments)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    do
                    {
                        if (args[index] == Path.DirectorySeparatorChar)
                        {
                            int cSlashes = 1;
                            index += 1;
                            while (index == args.Length && args[index] == Path.DirectorySeparatorChar)
                            {
                                cSlashes += 1;
                            }
                            ...
                            {
                                currentArg.Append(Path.DirectorySeparatorChar, cSlashes);
                            }
                            else
                            {
                                currentArg.Append(Path.DirectorySeparatorChar, (cSlashes >> 1));
                                if (0 != (cSlashes & 1))
                                {
                                    currentArg.Append('"');
                                }
        ...
        private class Argument
        {
            ...
            private bool ReportMissingRequiredArgument()
            {
                if (this.IsRequired && !this.SeenValue)
                {
                    ...
                    {
                        reporter(string.Format("Missing required argument '{1}{0}'.", this.LongName, (NativeMethods.IsRunningOnWindows) ? "/" : "--"));
                    }
